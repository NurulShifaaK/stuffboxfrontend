...
// const API_URL="https://mailbackend-5.onrender.com";
...
const Home = () => {
  ...
  const handlefile = (event) => {
    ...
    reader.onload = function (e) {
      ...
      const totalemail = emailData.map((item) => item.A);
      setEmailList(totalemail);
      console.log(totalemail);
    };
    reader.readAsArrayBuffer(file);
    ...
    axios
      ...
      .then((res) => {
        if (res.data.success) {
          ...
          alert("❌ Email Failed: " + (res.data.error || "Try again"));
        }
        setstatus(false);
      })
      .catch((err) => {
  ...
  return (
    <>
      <div className="flex flex-col justify-center max-w-xl mx-auto p-6 mt-10 bg-white/30 rounded-2xl shadow space-y-6">
        ...
        {/* Email Count */}
        <div>
          <p className="text-xl font-semibold text-black/60 text-center">
        ...
        <div className="flex justify-center">
          <button
            onClick={sendemail}
            className="bg-white/40 text-black/70 px-14 py-1 rounded"
          >
            {status ? "Sending..." : "Send"}
          </button>
        </div>
